function [ data, g, data0 ] = quad3D (accuracy)
...
% dataP1 = shapeRectangleByCorners(g, [-d; vrMin; v2Min], [d; vrMax; v2Max]);
% dataP2 = shapeHyperplane(g, [0;1;1], [0;0;v1Max]);
% dataP3 = shapeHyperplane(g, [0;-1;-1], [0;0;v1Min]);
% data = shapeIntersection(dataP1,dataP2);
% data = shapeIntersection(data, dataP3);
% data0 = data;

% ----------------- Unsafe conditions -----------------
% Below minimum relative distance, any relative velocity, any velocity
dataSD = shapeRectangleByCorners(g, [-d; -inf; -inf], [d; inf; inf]);
...
%   % Store new Value function and time stamp
%   
  V{idx}.time = tNow;
  V{idx}.value = data;
  idx = idx+1;
