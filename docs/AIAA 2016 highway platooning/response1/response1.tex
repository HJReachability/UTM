...
Good point. Here, liveness refers to the system eventually being able to reach a set of goal states. In the context of HJ reachability, the guarantee is actually being able to reach a set of goal states within some time horizon. The text has been modified to reflect this point, and a reference in the introduction has been added.
\end{itemize}
...
A continuous space is convenient since the cost map is defined on a geographic map, which is a continuous space.
...
\item \textit{What is the difference between the HJB and the HJ equations?}
