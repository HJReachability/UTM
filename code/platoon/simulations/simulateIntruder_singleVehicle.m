function simulateIntruder_singleVehicle(save_figures, fig_formats)
% simulateFormPlatoon()
%
...
tfm.aas{1}.plot_safe_V(intruder, tfm.qr_qr_safe_V, 2);
...
for i = 1:length(t)
  ...
  safe(end) = 1; % intruder vehicle is always safe
  for j = 1:length(tfm.aas)
    if safe(j)
