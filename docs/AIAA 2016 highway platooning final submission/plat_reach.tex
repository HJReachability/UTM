...
\noindent from which we obtain $\mathcal{V}(t) = \{x\in\mathbb{R}^n \mid V(t,x)\le 0\}$. The target set, reachable set, and implicit surface functions $l(x)$ and $V(t,x)$ representing them are shown in Fig. \ref{fig:RSExample}. From the solution $V(t,x)$, we can also obtain the optimal controls for both players via the following:
