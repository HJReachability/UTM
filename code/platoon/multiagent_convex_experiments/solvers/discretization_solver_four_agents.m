function controls = discretization_solver_four_agents(list_planes, safety_threshold, g, data, costates, dt)
    %%
...
for i=1:num_discretized
    for j=1:num_discretized
        for k=1:num_discretized
            ...
            dict = safety_dict('14');
            safety_matrix(1, 4) = dict(i, l);
            dict = safety_dict('41');
            safety_matrix(4, 1) = dict(l, i);
            dict = safety_dict('24');
            safety_matrix(2, 4) = dict(j, l);
            dict = safety_dict('42');
            safety_matrix(4, 2) = dict(l, j);
            dict = safety_dict('34');
            safety_matrix(3, 4) = dict(k, l);
            dict = safety_dict('43');
            safety_matrix(4, 3) = dict(l, k);
            ...
            %objective = min(min(matrix_penalize_1)) + alpha * min(min(matrix_penalize_2));
            %objective = sum(sum(matrix_penalize_1)) + alpha * max(max(matrix_penalize_2));
            objectives(i, j, k) = objective;
            
            if objective <= min_objective
                min_objective = objective;
                controls = [u_discretized(i) u_discretized(j) u_discretized(k) u_discretized(l)];
            end
