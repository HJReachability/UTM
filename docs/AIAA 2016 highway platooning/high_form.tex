...
\MCnote{Note that if vehicles in the airspace are far away from each other, it may be reasonable for all vehicles to remain in the ``free'' mode and fly in an unstructured manner. As long as multiple-way conflicts do not occur, pairwise collision avoidance maneuvers would be sufficient to ensure safety. Unstructured flight is likely to result in more efficient trajectories for each individual vehicle.}
