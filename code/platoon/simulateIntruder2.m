function simulateIntruder2(from_checkpoint, save_graphics, output_directory, visualize_vehicle_on)
...
%     open(writerObj);                            % Start videoWriter
    ...
    % Plot reach sets
    plot_qr = 2;
    qrs{plot_qr}.plotSafeV(qi, safeV);
    qrs{plot_qr}.plotSafeV(qrs{plot_qr-1}, safeV);
    qrs{plot_qr}.plotSafeV(qrs{plot_qr+1}, safeV);
...
for k = kStart:length(t)
    ...
    qrs{plot_qr}.plotSafeV(qi, safeV); %hsafeVI.LineColor = 'r';
    qrs{plot_qr}.plotSafeV(qrs{plot_qr-1}, safeV); %hsafeVFQ.LineColor = 'k';
    qrs{plot_qr}.plotSafeV(qrs{plot_qr+1}, safeV); %hsafeVBQ.LineColor = [0 0.5 0];
