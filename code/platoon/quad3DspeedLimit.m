function [ data, g, tau ] = quad3Dcollision (accuracy)
...
g.N = [ Nx; Nv; ceil(Nv/(g.max(2)-g.min(2))*(g.max(3)-g.min(3))) ];
g = processGrid(g);
g.dx
g.N
...
data = shapeRectangleByCorners(g, [-inf, -inf, v1Min], [inf, inf, v1Max]);
data = -data;
...
% return
v1slice = 0.95*v1Max;
[g2D, data2D] = proj2D(g, [0 0 1], g.N(1:2), data, v1slice);
[~, h1] = contour(g2D.xs{1}, g2D.xs{2}, data2D, [0 0],'r');
contour(g2D.xs{1}, g2D.xs{2}, data2D, [0 0],'r--');

[g2D, data2D] = proj2D(g, [0 0 1], g.N(1:2), data, 0);
