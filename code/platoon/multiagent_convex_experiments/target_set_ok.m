function [ data, g, data0 ] = target_set_ok(accuracy)
...
%   \dot \psi = a
%
% where v_a and evader is trying to get to the origin
%
...
singleStep = 1;              % Just for reachable target set
...
%---------------------------------------------------------------------------
% Initialize Display for level set visualization
f = figure;
...
% Construct a matrix with a large number (infinity first)
% As time goes on, set the data to the current time steps
% Also 

tNow = t0;
startTime = cputime;
timeData = 10000000 * ones(size(data));
while(tMax - tNow > small * tMax)
  ...
  % Update time data based on dt
  timeData(data <= 0) = min(timeData(data <= 0), tNow);
...
%%%%%%% Save relevant data
P = extractCostates(g, data, schemeData.innerData.derivFunc, true);
params = schemeData.innerData;

% Rename variable to avoid confusion with the avoidance reachability 
% computation
target_P = P;
target_data = data;
target_data0 = data0;
...
target_timeData = timeData;
save target_reachability.mat target_P target_data target_data0 ...
    target_g target_params target_timeData;
...
checkStructureFields(schemeData, 'grid', 'velocityA', 'inputA');

grid = schemeData.grid;

% Single player -- Hamiltonian
hamValue = -(schemeData.velocityA * cos(grid.xs{3}) .* deriv{1} ...
         + schemeData.velocityA * sin(grid.xs{3}) .* deriv{2} ...
         - schemeData.inputA * abs(deriv{3}));
