...
\end{aligned}
\end{equation}
\MCnote{The number of constraints here change with the decision variable $N$, but I'm ignoring this fact as the point is that this optimization is hard to solve, and therefore we solve the Eikonal equation instead.}

\noindent where $R(\cdot)$ is a regularizer, such as $R(N) = N^2$. \MCnote{Added regularizer to prevent $N\rightarrow\infty$}
